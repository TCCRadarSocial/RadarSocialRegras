...
angular
		...
		.controller(
				...
				function($scope, $http, $q, $rootScope) {
					...
					$scope.dataInicial = moment().subtract(1, 'days').utc()
							.format('YYYY-MM-DDTHH:mm:ss')
							+ '.000Z';
					$scope.dataFinal = moment().utc().format(
							'YYYY-MM-DDTHH:mm:ss')
							+ '.000Z';
					...
					$rootScope.buscar = function() {
						...
						if (angular.element(
								document.querySelector('#dataInicial')).val() != ""
								&& angular.element(
										document.querySelector('#dataFinal'))
										.val() != "") {

							$scope.dataInicial = angular.element(
									document.querySelector('#dataInicial'))
									.val();
							...
							$scope.dataFinal = angular.element(
									document.querySelector('#dataFinal')).val();
					...
					function carregaGraficoLinha() {
						...
						var baseQuery = '[{"portal": "' + $scope.portal
					...
					$scope.exportarExcel = function() {
						...
						alasql(
								'SELECT * INTO XLSX("relatorio_twitter.xlsx",{headers:true}) FROM ? ORDER BY Retweets DESC',
								[ dados ]);
